<x-layouts.default>
    <x-slot name="title">Edição de orçamentos</x-slot>

    <section class="container mx-auto">
        <header>
            <h1 class="text-2xl font-semibold text-gray-900">Editar orçamento</h1>
        </header>

        <div class="w-full mt-5">
            <form action="{{ route('orcamentos.update', ['budget' => $budget->id]) }}" method="POST" novalidate>
                @csrf
                @method('PUT')
                        ...
                        <div class="col-span-full sm:col-span-4">
                            ...
                            <x-select id="customer_id" name="customer_id">
                                ...
                                @foreach($customers as $customer)
                                    <option
                                        value="{{ $customer->id  }}"
                                        {{ old('customer_id') ?? $budget->customer_id == $customer->id ? 'selected' : ''  }}
                                    >
                                        {{ $customer->name }}
                                    </option>
                                @endforeach
                        ...
                        <div class="col-span-full sm:col-span-4">
                            ...
                            <x-select id="supplier_id" name="supplier_id">
                                ...
                                @foreach($suppliers as $supplier)
                                    <option
                                        value="{{ $supplier->id  }}"
                                        {{ old('supplier_id') ?? $budget->supplier->id == $supplier->id ? 'selected' : ''  }}
                                    >
                                        {{ $supplier->name }}
                                    </option>
                                @endforeach
                        ...
                        <div class="col-span-full sm:col-span-4">
                            ...
                            <x-select id="product_id" name="product_id">
                                ...
                                @foreach($products as $product)
                                    <option
                                        value="{{ $product->id  }}"
                                        {{ old('product_id') ?? $budget->product->id == $product->id ? 'selected' : ''  }}
                                    >
                                        {{ $product->name }}
                                    </option>
                                @endforeach
                        ...
                        <div class="col-span-3 sm:col-span-2">
                            ...
                            <div class="mt-1 flex rounded-md shadow-sm">
                                <x-input
                                    ...
                                    type="text"
                                    placeholder="1.000"
                                    value="{{ only_numbers(old('quantity') ?? $budget->quantity ) }}"
                                    class="flex-1 block w-full"
                                    required
                                    size="9"
                                    max="9"
                        ...
                        <div class="col-span-full sm:col-span-4">
                            ...
                            <x-select id="status" name="status">
                                ...
                                @foreach($budgetStatusOptions as $key => $status)
                                    <option
                                        value="{{ $key  }}"
                                        {{ old('status') ?? $budget->status == $key ? 'selected' : ''  }}
                                    >
                                        {{ $status }}
                                    </option>
                                @endforeach
